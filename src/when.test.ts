...
const errMsg = ({ expect, actual }: { expect: any; actual: any }) =>
...
describe('When', () => {
  ...
  describe('when', () => {
    ...
    it('reset of mocks restores original implementation', () => {
      const fn = jest.fn((_num: number) => 'a');
    ...
    it('fails verification check if all mocks were not called with line numbers', () => {
      ...
      try {
        ...
        const currentFilePathPattern = /src(?:\\|\/)when\.test\.ts:\d{3}(.|\s)*/;
        const numberOfMatches = errorLines.filter((line: string) => currentFilePathPattern.test(line)).length;
  ...
  describe('mock implementation', () => {
    ...
    describe('function matcher', () => {
      it('works with custom function args', () => {
        ...
        const allValuesTrue = (arg: any) => Object.values(arg).every(Boolean);
        const numberDivisibleBy3 = (arg: number) => arg % 3 === 0;
      ...
      it('custom function args get access to the "equals" jasmine util', () => {
        ...
        const arrMatch = (arg: any, equals: jest.MatcherUtils['equals']) => equals(arg, [1, 2, 3]);
    ...
    describe('when.allArgs', () => {
      ...
      it('allows matching against all the args at once with when.allArgs', () => {
        ...
        const numberDivisibleBy3 = (args: number[]) => args.every(arg => arg % 3 === 0);
      ...
      it('all args are numbers example', () => {
        const fn = jest.fn();
        const areNumbers = (args: any[], equals: jest.MatcherUtils['equals']) => args.every(arg => equals(arg, expect.any(Number)));
      ...
      it('first matcher is a proxy', () => {
        ...
        expect((proxy as FunctionMatcher)._isAllArgsFunctionMatcher).toEqual(true);
      ...
      it('single arg match example', () => {
        const fn = jest.fn();
        const argAtIndex = (index: number, matcher: any) => when.allArgs((args: any[], equals: jest.MatcherUtils['equals']) => equals(args[index], matcher));
      ...
      it('partial match example', () => {
        const fn = jest.fn();
        const partialArgs = (...argsToMatch: any[]) => when.allArgs((args: any[], equals: jest.MatcherUtils['equals']) => equals(args, expect.arrayContaining(argsToMatch)));
      ...
      it('react use case from github', () => {
        ...
        const propsOf = (propsToMatch: any) => when.allArgs(([props, refOrContext]: any[], equals: jest.MatcherUtils['equals']) => equals(props, propsToMatch));
    ...
    describe('Default Behavior (no called with)', () => {
      describe('defaultX methods', () => {
        ...
        it('has access to args in a default implementation (defaultReturnValue alias)', () => {
          ...
          when(fn)
            .defaultImplementation(({ name }: { name: string }) => `Hello ${name}`)
        ...
        it('preserves a prior bare .calledWith() when followed by .defaultReturnValue()', () => {
          const fn = jest.fn();
        ...
        it('preserves a prior .calledWith(undefined) registration when followed by .defaultReturnValue()', () => {
          const fn = jest.fn();
        ...
        it('preserves a prior .calledWith(undefined, undefined) registration when followed by .defaultReturnValue()', () => {
          const fn = jest.fn();
        ...
        it('still allows .defaultReturnValue() to be replaced by a later .defaultReturnValue()', () => {
          const fn = jest.fn();

          when(fn)
            .defaultReturnValue('first')
            .defaultReturnValue('second');

          expect(fn('anything')).toBe('second');
        });

        it('preserves a prior bare .calledWith() when followed by .defaultResolvedValue()', async () => {
          const fn = jest.fn();

          when(fn)
            .calledWith().mockResolvedValue('no-args')
            .defaultResolvedValue('default');
        ...
        it('preserves a prior .calledWith(undefined) registration when followed by .defaultResolvedValue()', async () => {
          const fn = jest.fn();

          when(fn)
            .calledWith(undefined).mockResolvedValue('one-undef')
            .defaultResolvedValue('default');
        ...
        it('preserves a prior .calledWith(undefined, undefined) registration when followed by .defaultResolvedValue()', async () => {
          const fn = jest.fn();

          when(fn)
            .calledWith(undefined, undefined).mockResolvedValue('two-undef')
            .defaultResolvedValue('default');
        ...
        it('still allows .defaultResolvedValue() to be replaced by a later .defaultResolvedValue()', async () => {
          const fn = jest.fn();

          when(fn)
            .defaultResolvedValue('first')
            .defaultResolvedValue('second');

          await expect(fn('anything')).resolves.toBe('second');
        });

        it('preserves a prior bare .calledWith() when followed by .defaultRejectedValue()', async () => {
          const fn = jest.fn();
        ...
        it('preserves a prior .calledWith(undefined) registration when followed by .defaultRejectedValue()', async () => {
          const fn = jest.fn();
        ...
        it('preserves a prior .calledWith(undefined, undefined) registration when followed by .defaultRejectedValue()', async () => {
          const fn = jest.fn();
        ...
        it('still allows .defaultRejectedValue() to be replaced by a later .defaultRejectedValue()', async () => {
          const fn = jest.fn();

          when(fn).defaultRejectedValue(new Error('first'));
          await expect(fn('anything')).rejects.toThrow('first');

          when(fn).defaultRejectedValue(new Error('second'));
          await expect(fn('anything')).rejects.toThrow('second');
        });

        it('preserves a prior bare .calledWith() when followed by a default-routed .mockImplementation()', () => {
          const fn = jest.fn();

          when(fn).calledWith().mockReturnValue('no-args');
          when(fn).mockImplementation(() => 'default');

          expect(fn()).toBe('no-args');
          expect(fn('x')).toBe('default');
        });

        it('preserves a prior .calledWith(undefined) registration when followed by a default-routed .mockImplementation()', () => {
          const fn = jest.fn();
        ...
        it('preserves a prior .calledWith(undefined, undefined) registration when followed by a default-routed .mockImplementation()', () => {
          const fn = jest.fn();

          when(fn).calledWith(undefined, undefined).mockReturnValue('two-undef');
          when(fn).mockImplementation(() => 'default');
        ...
        it('preserves a prior bare .calledWith() when followed by a default-routed .mockReturnValue()', () => {
          const fn = jest.fn();

          when(fn).calledWith().mockReturnValue('no-args');
          when(fn).mockReturnValue('default');

          expect(fn()).toBe('no-args');
          expect(fn('x')).toBe('default');
        });

        it('preserves a prior .calledWith(undefined) registration when followed by a default-routed .mockReturnValue()', () => {
          const fn = jest.fn();
        ...
        it('preserves a prior .calledWith(undefined, undefined) registration when followed by a default-routed .mockReturnValue()', () => {
          const fn = jest.fn();
        ...
        it('preserves a prior bare .calledWith() when followed by a default-routed .mockResolvedValue()', async () => {
          const fn = jest.fn();

          when(fn).calledWith().mockResolvedValue('no-args');
          when(fn).mockResolvedValue('default');

          await expect(fn()).resolves.toBe('no-args');
          await expect(fn('x')).resolves.toBe('default');
        });

        it('preserves a prior .calledWith(undefined) registration when followed by a default-routed .mockResolvedValue()', async () => {
          const fn = jest.fn();
        ...
        it('preserves a prior .calledWith(undefined, undefined) registration when followed by a default-routed .mockResolvedValue()', async () => {
          const fn = jest.fn();

          when(fn).calledWith(undefined, undefined).mockResolvedValue('two-undef');
          when(fn).mockResolvedValue('default');
        ...
        it('preserves a prior bare .calledWith() when followed by a default-routed .mockRejectedValue()', async () => {
          const fn = jest.fn();

          when(fn).calledWith().mockResolvedValue('no-args');
          when(fn).mockRejectedValue(new Error('default'));

          await expect(fn()).resolves.toBe('no-args');
          await expect(fn('x')).rejects.toThrow('default');
        });

        it('preserves a prior .calledWith(undefined) registration when followed by a default-routed .mockRejectedValue()', async () => {
          const fn = jest.fn();
        ...
        it('preserves a prior .calledWith(undefined, undefined) registration when followed by a default-routed .mockRejectedValue()', async () => {
          const fn = jest.fn();
      ...
      describe('legacy methods', () => {
        ...
        it('has access to args in a default implementation', () => {
          ...
          when(fn)
            .mockImplementation(({ name }: { name: string }) => `Hello ${name}`)
            .calledWith({ name: 'bar' }).mockReturnValue('Goodbye bar');
    ...
    it('allows using mockImplementation', () => {
      ...
      when(fn).calledWith('foo', 'bar').mockImplementation((...args: any[]) => args);
    ...
    it('allows using mockImplementationOnce', () => {
      ...
      when(fn).calledWith('foo', 'bar').mockImplementationOnce((...args: any[]) => args);
    ...
    it('keeps call context when not matched', () => {
      class TheClass {
        ...
        request(...args: any[]) {
          return (this.call as (...args: any[]) => any)(...args);
        }
      }
    ...
    it('keeps call context when matched', () => {
      ...
      when(theSpiedMethod)
        .calledWith(1)
        .mockImplementation(function (this: TheClass) {
          return this.call() + '!';
        });
    ...
    it('allows reset of specific mock with specific args by just setting to undefined', () => {
      ...
      expect((mock as WhenMockWithInternals).__whenMock__.callMocks).toHaveLength(1);
      when(mock).calledWith('hello').mockReturnValue(undefined);
      expect((mock as WhenMockWithInternals).__whenMock__.callMocks).toHaveLength(1);
      when(mock).calledWith('hello').mockReturnValueOnce('a');
      expect((mock as WhenMockWithInternals).__whenMock__.callMocks).toHaveLength(2);
      when(mock).calledWith('hello').mockReturnValueOnce('b');
      expect((mock as WhenMockWithInternals).__whenMock__.callMocks).toHaveLength(3);

      expect(mock('hello')).toEqual('a');
      expect(mock('hello')).toEqual('b');
      expect(mock('hello')).toEqual(undefined);

      console.log((mock as WhenMockWithInternals).__whenMock__.callMocks);
    ...
    it('allows reset of specific mock with specific args by using mockReset after calledWith', () => {
      ...
      expect((mock as WhenMockWithInternals).__whenMock__.callMocks).toHaveLength(0);
      when(mock).calledWith('hello').mockReturnValueOnce('a');
      expect((mock as WhenMockWithInternals).__whenMock__.callMocks).toHaveLength(1);
      when(mock).calledWith('hello').mockReturnValueOnce('b');
      expect((mock as WhenMockWithInternals).__whenMock__.callMocks).toHaveLength(2);
